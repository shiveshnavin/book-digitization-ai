...
def normalize_topic(topic: str) -> str:
    ...
    topic_str = re.sub(r"\bPara\s*[-–]?\s*athletics\b", "Para Athletics", topic_str, flags=re.IGNORECASE)
    topic_str = re.sub(r"\bImportant\s+Days\b", "Important Days And Themes", topic_str, flags=re.IGNORECASE)
...
def manual_process(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        item["exam"] = "ssc"
        item["subject"] = "general_awareness" 
