...
def normalize_topic(topic: str) -> str:
    ...
    topic_str = re.sub(r"\bDBMS\b", "DBMS", topic_str, flags=re.IGNORECASE)
    topic_str = re.sub(r"\bIP\b", "IP", topic_str, flags=re.IGNORECASE)
    topic_str = topic_str.title()
    # Restore specific acronyms
    topic_str = re.sub(r"\bDbms\b", "DBMS", topic_str)
    topic_str = re.sub(r"\bIp\b", "IP", topic_str)
    return topic_str
...
def manual_process(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        item["exam"] = "ssc"
        item["subject"] = "computer_knowledge" 
