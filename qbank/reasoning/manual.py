...
def normalize_topic(topic: str) -> str:
    ...
    topic_str = re.sub(r"\bVenn\s*[-–]?\s*Diagram\b", "Venn Diagram", topic_str, flags=re.IGNORECASE)
    topic_str = re.sub(r"\bAnalogy\s+Non\s*[-–]?\s*Verbal\b", "Analogy Non-Verbal", topic_str, flags=re.IGNORECASE)
    topic_str = re.sub(r"\bSeries\s+Non\s*[-–]?\s*Verbal\b", "Series Non-Verbal", topic_str, flags=re.IGNORECASE)
    topic_str = re.sub(r"\bCounting\s+of\s+figure[s]?\b", "Counting Of Figures", topic_str, flags=re.IGNORECASE)
    topic_str = re.sub(r"\bOrder\s+(&|and)\s+Ranking\b", "Order And Ranking", topic_str, flags=re.IGNORECASE)
